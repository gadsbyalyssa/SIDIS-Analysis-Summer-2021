...
c04->cd(1);
SinPosDP1->GetXaxis()->SetTitle("Sin(deltaphi+) bin1");
...
c05->cd(1);
SinNegDP1->GetXaxis()->SetTitle("Sin(deltaphi-) bin1");
...
c06->cd(1);
SinSqrDP1->GetXaxis()->SetTitle("Sin^{2}(deltaphi) bin1 ");
...
ofstream outputfile("MomentsMethodBin1.txt");
